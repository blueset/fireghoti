...
export class UserConverter {
    public static async encode(u: User, ctx: MastoContext): Promise<MastodonEntity.Account> {
        ...
        return cache.locks.acquire(u.id, async () => {
            ...
            const bio = Promise.resolve(profile).then(async profile => {
            ...
            const fields = Promise.resolve(profile).then(profile => Promise.all(profile?.fields.map(async p => this.encodeField(p, u.host, profile?.mentions)) ?? []));
