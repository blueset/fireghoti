...
export class NotificationConverter {
    ...
    public static async encodeNotificationTypeOrDefault(t: NotificationType): Promise<string> {
        try {
    ...
    public static async encodeSubscription(subscription: SwSubscription, ctx: MastoContext): Promise<MastodonEntity.PushSubscription> {
        const instance = await fetchMeta(true);
        return {
            id: subscription.id,
            endpoint: subscription.endpoint,
            server_key: instance.swPublicKey ?? "",
            alerts: { // FIXME: Implement alerts
                follow: true,
                favourite: true,
                mention: true,
                reblog: true,
    ...
    public static async encodePushNotificationPayload<T extends keyof pushNotificationsTypes>(
        ...
        if (type === "notification") {
            ...
            notification_type = await this.encodeNotificationTypeOrDefault(notificationBody.type);
            const effectiveNote = notificationBody.note?.renote ?? notificationBody.note;

            icon = notificationBody.user?.avatarUrl ?? notificationBody.note?.user.avatarUrl ?? notificationBody.icon ?? undefined;
            const displayName = 
            ...
            switch (notificationBody.type) {
                case "mention": title = `${displayName} mentioned you`; break;
                case "reply": title = `${displayName} replied to you`; break;
                case "renote": title = `${displayName} boosted your note`; break;
                case "quote": title = `${displayName} quoted your note`; break;
                case "reaction": title = `${displayName} reacted to your note`; break;
                case "pollVote": title = `${displayName} voted ${notificationBody.note?.poll.choices[notificationBody.choice || 0]?.text}`; break;
                case "pollEnded": title = `${displayName} closed a poll`; break;
                case "followRequestAccepted": title = `${displayName} accepted your follow request`; break;
                case "groupInvited": title = `${displayName} invited you to ${notificationBody.invitation.group.name}`; break;
                case "app": title = `${notificationBody.header}`; break;
                default: title = `New ${type} (${notificationBody.type}) notification`;
            }
            description = (effectiveNote && getNoteSummary(effectiveNote)) || notificationBody.body || "";
        } else if (type === "unreadMessagingMessage") {
            const notificationBody = body as pushNotificationsTypes["unreadMessagingMessage"];
            notification_id = notificationBody.id;
