...
import { DriveFiles, Metas, NoteEdits, NoteFavorites, NoteReactions, Notes, Polls, UserNotePinings } from "@/models/index.js";
...
export class NoteHelpers {
    ...
    public static normalizeEditOptions(body: any): MastodonEntity.StatusEditRequest {
        ...
        if (body.media_attributes)
            result.media_attributes = body.media_attributes && body.media_attributes.length > 0
    ...
    public static async translateNote(status: MastodonEntity.Status, targetLang: string, ctx: MastoContext): Promise<MastodonEntity.StatusTranslation> {
        const user = ctx.user as ILocalUser;
	    const instance = await fetchMeta();
        const provider = instance.libreTranslateApiUrl ? 'LibreTranslate' : instance.deeplAuthKey ? 'DeepL' : undefined;
        if (provider === undefined) throw new Error('No translator is set up on this server.');

        let sourceLang = status.language as PostLanguage ?? "";
        let translatedText = "";
        if (status.content) {
            const translated = await translate(status.content, sourceLang, targetLang as PostLanguage);
            translatedText = translated.text;
            sourceLang = translated.sourceLang as PostLanguage ?? sourceLang;
        }

        const translateText = (text: string) => translate(text, sourceLang, targetLang as PostLanguage).then(p => p.text);

        return await awaitAll({
            content: translatedText,
            spoiler_text: status.spoiler_text ? translateText(status.spoiler_text) : null,
            media_attachments: Promise.all(status.media_attachments.map(async (s) => s.description ? translateText(s.description) : null)),
            poll: status.poll ? awaitAll({
                id: status.poll.id, 
                options: Promise.all(status.poll.options.map(async (option) => {
