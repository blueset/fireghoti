...
const mutedWordsCache = new Cache<Pick<UserProfile, "mutedWords" | "mutedPatterns">>(
	"mutedWords",
	60 * 5,
);
...
export async function isFiltered(
	...
	const profile = await mutedWordsCache.fetch(user.id, () =>
		UserProfiles.findOneBy({ userId: user.id }).then(
			(p) => ({ mutedWords: p?.mutedWords ?? [], mutedPatterns: p?.mutedPatterns ?? [] }),
		),
	);

	if (!profile || (profile.mutedPatterns.length < 1 && profile.mutedWords.length < 1)) return false;
